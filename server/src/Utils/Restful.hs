...
renderEntityList :: (MonadHandler m, ToJSON v, ToJSON (Entity t)) => Route (HandlerSite m) -> (Key t -> Route (HandlerSite m)) -> [Entity t] -> v -> m Value
renderEntityList listRoute partialEntityRoute items totalCount = do
    urlRender <- getUrlRender
    let itemsWithEntityMetaData = fmap (\(Entity entityId entity) -> addEntityMetaData urlRender partialEntityRoute entityId entity) items :: [Maybe (HashMap Text Value)]
...
returnValueOrThrowException :: MonadIO m => Either Text a -> m a
...
addPager :: Maybe Text -> Int -> [SelectOpt record] -> Either Text [SelectOpt record]
addPager mpage resultsPerPage selectOpt =
    case getCurrentPage mpage of
        Left val -> Left val
        Right pageNumber -> Right $ selectOpt ++ limitTo ++ offsetBy
            where limitTo = [LimitTo resultsPerPage]
                  offsetBy = [OffsetBy $ (pageNumber - 1) * resultsPerPage | pageNumber > 0]

addListMetaData urlRender route totalCount keyValues = keyValues ++ metaData
  where
...
addEntityMetaData urlRender route entityId entity =
    case toJSON (Entity entityId entity) of
        ...
        _ -> Nothing
  where
    self = String $ urlRender (route entityId)

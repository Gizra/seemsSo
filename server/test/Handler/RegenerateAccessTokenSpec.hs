...
module Handler.RegenerateAccessTokenSpec
    ( spec
    ) where
    ...
    withApp $ do
        describe "Regenerate access tokenR" $ do
            ...
            it "should user to access own regenerate access token page" $ do
                ...
                statusIs 200
            it
                "should not allow user to access another user's regenerate access token page" $ do
                userEntity <- createUser "alice"
                let (Entity uid _) = userEntity
                authenticateAs userEntity
                ...
                statusIs 403
            it
                "should allow admin to access regenerate access token page on behalf of another user" $
          -- Create "admin" role
             do
